  ...
  try {
    ...
    if (!checkFolderWritePermission(backuppath)) {
      ...
      refLog.logData.push({
        color: "red",
        Message:
          "Backup Failed: No write permissions for the folder: " + backuppath,
      });
      refLog.logData.push({
        color: "red",
    ...
    const directoryPath = path.join(
      ...
      "..",
      backupfolder,
      `backup_${now.format("yyyy-MM-DD HH-mm-ss")}.json`,
    );
    ...
    refLog.logData.push({
      color: "yellow",
      Message: "Begin Backup " + directoryPath,
    });
    for (let table of tables) {
      ...
      refLog.logData.push({
        color: "dodgerblue",
        Message: `Saving ${rows.length} rows for table ${table}`,
      });
    ...
    for (var oldBackup of fileData) {
      ...
      refLog.logData.push({
        color: "yellow",
        Message: `${oldBackupFile} has been deleted.`,
      });
    }

    refLog.logData.push({
      color: "lawngreen",
      Message: deleteCount + " backups removed.",
    });
  } catch (error) {
    console.log(error);
    refLog.logData.push({ color: "red", Message: "Backup Failed: " + error });
