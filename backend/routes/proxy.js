...
router.get('/Items/Images/Backdrop/', async(req, res) => {
    ...
    let url=`${config.JF_HOST}/Items/${id}/Images/Backdrop?fillWidth=${fillWidth || 800}&quality=${quality || 100}&blur=${blur || 0}`;
  ...
  router.get("/getRecentlyAdded", async (req, res) => {
    try {
      ...
      const recentlyAdded = await Jellyfin.getRecentlyAdded(libraryid);
