...
export default function Navbar() {
  ...
  return (
    <BootstrapNavbar
      variant="dark"
      className=" d-flex flex-column py-0 text-center sticky-top"
    >
        ...
        <Nav className="flex-row flex-md-column w-100">
          {navData.map((item) => {
            ...
            const isActive =
              ...
              ((locationString.length > 0 && item.link.length > 0) ||
                (locationString.length === 1 && item.link.length === 0)); // check if the link is the current path
            return (
              <Nav.Link
                as={Link}
                key={item.id}
