      ...
      <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
        <TableCell>
          ...
          >
            {!open ? (
              <AddCircleFillIcon
                opacity={data.Log.length > 0 ? 1 : 0}
                cursor={data.Log.length > 1 ? "pointer" : "default"}
              />
            ) : (
        ...
        <TableCell>
          {Intl.DateTimeFormat("en-UK", options).format(new Date(data.TimeRun))}
        </TableCell>
          ...
          <div
            className={`badge ${
              data.Result.toLowerCase() === "success"
                ? "text-bg-success"
                : data.Result.toLowerCase() === "running"
                ? "text-bg-warning"
                : "text-bg-danger "
              ...
              <Table aria-label="sub-activity" className="rounded-2">
                <TableBody>
                  <TableRow key={data.Id}>
                    <TableCell colSpan="7">
                      <TerminalComponent data={data.Log} />
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
          ...
          <TableBody>
            {data &&
              data
                ...
                .map((log, index) => <Row key={index} data={log} />)}
            {data.length === 0 ? (
              <tr>
                <td
                  colSpan="7"
